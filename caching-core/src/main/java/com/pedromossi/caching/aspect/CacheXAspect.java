...
/**
 * Spring AOP aspect that implements caching behavior for methods annotated with {@link CacheX}.
 *
 * <p>This aspect intercepts method calls to {@code @CacheX} annotated methods and applies
 * the appropriate caching logic based on the operation type. It serves as the bridge between
 * the declarative {@code @CacheX} annotation and the underlying {@link CacheService}.</p>
 *
 * <p><strong>Key Responsibilities:</strong></p>
 * <ul>
 * <li><strong>Method Interception:</strong> Uses AOP to intercept annotated method calls</li>
 * <li><strong>SpEL Evaluation:</strong> Parses and evaluates cache key expressions</li>
 * <li><strong>Type Safety:</strong> Preserves method return types for cache operations</li>
 * <li><strong>Operation Routing:</strong> Delegates to appropriate cache operations (GET/EVICT)</li>
 * <li><strong>Error Handling:</strong> Ensures method execution even if cache operations fail</li>
 * </ul>
 *
 * <p><strong>Execution Flow:</strong></p>
 * <ol>
 * <li>AOP proxy intercepts method call to {@code @CacheX} annotated method</li>
 * <li>Aspect evaluates SpEL expression to generate cache key</li>
 * <li>Routes to appropriate operation handler based on annotation configuration</li>
 * <li>For GET: Check cache, execute method if miss, store result</li>
 * <li>For EVICT: Execute method first, then invalidate cache</li>
 * <li>Return method result to caller</li>
 * </ol>
 *
 * <p><strong>SpEL Integration:</strong></p>
 * <p>The aspect uses Spring's Expression Language (SpEL) for dynamic cache key generation.
 * It creates a {@link MethodBasedEvaluationContext} that provides access to:</p>
 * <ul>
 * <li>Method parameters by name (e.g., {@code #userId})</li>
 * <li>Method target object</li>
 * <li>Method metadata and argument array</li>
 * <li>All standard SpEL functions and operators</li>
 * </ul>
 *
 * <p><strong>Type Safety Implementation:</strong></p>
 * <p>The aspect automatically extracts the generic return type from the intercepted method
 * using {@link ParameterizedTypeReference}, ensuring type-safe cache operations without
 * requiring explicit type declarations in the annotation.</p>
 *
 * <p><strong>Error Resilience:</strong></p>
 * <p>If cache operations fail, the aspect ensures that the original method is still executed,
 * maintaining application functionality even when cache infrastructure is unavailable.</p>
 *
 * <p><strong>Configuration Requirements:</strong></p>
 * <ul>
 * <li>Spring AOP must be enabled ({@code @EnableAspectJAutoProxy})</li>
 * <li>This class must be registered as a Spring bean</li>
 * <li>Target classes must be Spring-managed beans</li>
 * <li>Annotated methods must be public</li>
 * </ul>
 *
 * <p><strong>Performance Considerations:</strong></p>
 * <p>The aspect adds minimal overhead to method calls. SpEL expressions are parsed once
 * and cached internally by Spring's expression parser. The main performance impact comes
 * from the underlying cache operations, not the aspect itself.</p>
 *
 * @since 1.1.0
 * @see CacheX
 * @see CacheService
